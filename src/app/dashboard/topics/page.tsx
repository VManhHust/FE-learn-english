...
function LessonCard({
  ...
  return (
    <div
      className="group flex cursor-pointer flex-col overflow-hidden rounded-xl border border-gray-200 shadow-sm transition-all duration-300 ease-out hover:-translate-y-1 hover:border-[#dfbd78] hover:shadow-[0_16px_36px_rgba(72,58,31,0.14)] active:translate-y-0 active:scale-[0.99] motion-reduce:transform-none motion-reduce:transition-none dark:border-[#1a1a1a] dark:hover:border-[#6b5428] dark:hover:shadow-[0_16px_36px_rgba(0,0,0,0.28)]"
      style={{ boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06)' }}
      onClick={handleClick}
    >
      <div className="relative flex-shrink-0 overflow-hidden" style={{ backgroundColor: bg, height: 120 }}>
        {thumbnail ? (
          <img
            src={thumbnail}
            alt={lesson.title}
          ...
          <div className="flex h-full w-full items-center justify-center text-4xl opacity-30 transition-transform duration-500 ease-out group-hover:scale-110 motion-reduce:transform-none motion-reduce:transition-none">
      ...
      <div className="relative flex flex-1 flex-col bg-white p-3 transition-colors duration-300 group-hover:bg-[#fffdf8] dark:bg-[#1a1a1a] dark:group-hover:bg-[#211d16]" style={{ minHeight: 72 }}>
        <p className="line-clamp-2 flex-1 text-xs font-medium leading-snug text-gray-900 transition-colors duration-300 group-hover:text-[#9a6b18] dark:text-gray-100 dark:group-hover:text-[#d4b05a]">
          {lesson.title}
        </p>
      ...
      <div className="flex flex-wrap items-center gap-2 sm:gap-3">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              ...
              <svg width="12" height="12" viewBox="0 0 12 12" fill="currentColor" className="transition-transform duration-200 group-data-[state=open]:rotate-180">
                <path d="M6 8L1 3h10z" />
              </svg>
            </button>
          </DropdownMenuTrigger>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-64 rounded-xl bg-[#f5f3ef] dark:bg-[#1a1917] border border-gray-200 dark:border-[#1a1a1a] p-0 max-h-72 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            <div className="px-4 py-2 border-b border-gray-200 dark:border-[#1a1a1a] flex items-center justify-between sticky top-0 bg-[#f5f3ef] dark:bg-[#1a1917]">
              ...
              {activeTags.length > 0 && (
                <button onClick={() => setActiveTags([])} className="rounded px-1 text-xs transition-all duration-200 hover:bg-[#fff8e8] hover:underline active:scale-95 motion-reduce:transform-none" style={{ color: '#d4a853' }}>
                  {t.clearAll}
                </button>
              )}
            </div>
              ...
              {progressFilters.length > 0 && (
                <span className="flex items-center gap-0.5">
                  {progressFilters.slice(0, 3).map(filter => {
                    ...
                    return (
                    <span key={filter} className="h-2 w-2 animate-in zoom-in-50 rounded-full" style={{ backgroundColor: filterObj?.color }} />
                    )
                  })}
                </span>
              )}
        ...
        {(levelFilters.length > 0 || activeTags.length > 0 || progressFilters.length > 0) && (
          <div className="flex flex-wrap items-center gap-2">
            ...
            {activeTags.map(tag => (
              <Badge key={tag} className="flex items-center gap-1 rounded-full bg-gray-800 px-3 py-1 text-xs font-semibold text-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md active:translate-y-0 active:scale-[0.98] motion-reduce:transform-none motion-reduce:transition-none dark:bg-gray-200 dark:text-gray-900">
                {tag}
                <button onClick={() => setActiveTags(activeTags.filter(t => t !== tag))} className="ml-1 rounded-full transition-transform duration-200 hover:scale-125 hover:opacity-80 active:scale-95 motion-reduce:transform-none">✕</button>
              </Badge>
            ))}
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    className="ml-1 rounded-full transition-transform duration-200 hover:scale-125 hover:opacity-80 active:scale-95 motion-reduce:transform-none"
                  >
                    ✕
                  </button>
                </Badge>
            ...
            <button
              ...
              className="rounded px-1 text-xs text-gray-400 underline transition-all duration-200 hover:-translate-y-0.5 hover:text-gray-600 active:translate-y-0 active:scale-95 motion-reduce:transform-none dark:hover:text-gray-200"
            >
              {t.clearAll}
            </button>
          </div>
